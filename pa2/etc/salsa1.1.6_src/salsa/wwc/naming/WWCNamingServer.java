...
// cvarela 20210923: this line is causing an infinite loop in each name server thread!
// Creating persistent socket connections requires a newer UANP protocol...
// For now, I am only allowing one request per connection.
//      if (version==null) {continue;}
        String command = ins.readLine();
        String parameter1 = ins.readLine();
        String parameter2 = ins.readLine();
        if ((version==null) || (!version.equals(UANProtocol.VERSION))) {
          outs.println(UANProtocol.VERSION + "\n"
                       + UANProtocol.BAD_REQ_STATUS_CODE + "\n"
                       + "Wrong Version" + "\n"
                       );
// cvarela: as soon as a null or incorrect protocol version is received in socket
...
//cvarela: clean up after exiting loop
    } finally {
    	try {	
    		outs.close();
    		ins.close();
